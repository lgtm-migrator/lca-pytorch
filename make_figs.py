...
sparsity = np.sort((acts != 0).mean(axis=(0, 2, 3, 4)))[::-1]
plt.plot(sparsity)
...
rgrids = [make_grid(recons[:, :, t], nrow=int(sqrt(recons.shape[0]))) for t in range(recons.shape[2])]
igrids = [make_grid(inputs[:, :, t], nrow=int(sqrt(inputs.shape[0]))) for t in range(inputs.shape[2])]
